...
def plot_result(Z, title, show=True, save_plot=None, xlabel=None, ylabel=None):
    plt.plot(Z)
...
def distance_precision_plot(bboxes, ground_truth, title, show=True, save_plot=None):
    ...
    distances = np.sqrt(np.square(positions[:, 0] - ground_truth[:, 0]) + np.square(positions[:, 1] - ground_truth[:, 1]))
